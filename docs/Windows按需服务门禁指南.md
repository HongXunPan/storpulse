...
这是一条只面向 Windows 10 22H2 x64 的最小候选门禁，用于验证“普通用户客户端按需启动 LocalSystem 服务，由服务采集 ETW DiskIo，再把聚合结果返回客户端”是否可行。它不改变标准用户 `StartTraceW=5` 的失败结论，也不表示 Windows 阶段 1–3、Windows 11、正式安装或正式发布已经放行。
...
2. 双击 `安装按需服务门禁.cmd`，在 UAC 中允许一次。该步骤只复制经过包内 SHA-256 校验的探针并创建手动服务。
3. 关闭提升窗口，确认文件管理器和当前终端没有“以管理员身份运行”。
4. **先**双击 `验证异常断开清理.cmd`，等待生成第一个 `storpulse-diagnostics-*.zip`。该入口在服务报告就绪后主动断开客户端。
5. **再**双击 `收集按需服务门禁日志.cmd`，等待约 15–30 秒并生成第二个 ZIP。第二次仍能启动同名 ETW 会话，才可间接证明上一步没有遗留会话。
6. 双击 `卸载按需服务门禁.cmd`，再次允许 UAC，看到服务和受保护目录已清理后再删除解压目录。
7. 把两个 ZIP 返回，并标注“异常断开”和“正常采集”；不要发送 EXE、PDB、整个解压目录、截图或 Windows 事件日志。

如果任一步失败，不要手工运行 EXE、不要修改服务权限，也不要重复安装覆盖。保留已生成的 ZIP；若卸载失败，记录屏幕上的错误阶段并停止。
