...
本指南用于在 Windows 10 22H2 x64 上验证 StorPulse 产品服务的持续协议、真实 ETW 聚合、普通用户启动、正常停止、客户端断开、连接超时、未交付终止失败后备记录、客户端强杀清理和传统待机恢复。当前 Win10 Pro 22H2 x64 传统待机（S3）与服务后备记录均已取得标准用户实机通过结果；实现或测试入口变化后仍按本文独立复验。它是 Windows 阶段 1 实机门禁，不等于 Windows 11、Modern Standby、WinUI、签名安装、长期运行或正式发布已经通过。
