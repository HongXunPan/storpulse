...
pub(super) fn start(nonce: &str) -> Result<(), ClientError> {
    ...
    if unsafe { StartServiceW(service.handle, arguments.len() as u32, arguments.as_ptr()) } == 0 {
        return Err(last_error("service_start_failed"));
    }
...
impl ServiceHandle {
    fn open_manager() -> Result<Self, ClientError> {
        // SAFETY：空机器名和数据库名表示本机活动 SCM 数据库。
        let handle =
            unsafe { OpenSCManagerW(std::ptr::null(), std::ptr::null(), SC_MANAGER_CONNECT) };
        if handle.is_null() {
            return Err(last_error("open_service_manager_failed"));
        }
        Ok(Self { handle })
    }

    fn open_service(&self, access: u32) -> Result<Self, ClientError> {
        let name = wide(PRODUCT_SERVICE_NAME);
        // SAFETY：SCM 句柄有效，服务名以 NUL 结尾。
        let handle = unsafe { OpenServiceW(self.handle, name.as_ptr(), access) };
        if handle.is_null() {
            return Err(last_error("open_service_failed"));
        }
        Ok(Self { handle })
    }

    fn query_status(&self) -> Result<SERVICE_STATUS_PROCESS, ClientError> {
        ...
        if queried == 0 {
            return Err(last_error("query_service_status_failed"));
        }
        Ok(status)
    }
